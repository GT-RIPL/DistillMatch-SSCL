...
###############################################################
# save directory
OUTDIR=outputs/CIFAR100-10k/uniform_${SPLIT}
MAXTASK=-1
...
# base
python -u run_sscl.py --dataset CIFAR100 --l_dist $L_DIST --train_aug --rand_split --gpuid $GPUID --repeat 2 --labeled_samples 10000 --unlabeled_task_samples -1 \
    --force_out_dim 100 --first_split_size $SPLIT --other_split_size $SPLIT  --schedule $SCHEDULE_GD --schedule_type decay --batch_size $BS --ul_batch_size $UBS   \
    --optimizer SGD --lr $LR_PL --momentum 0.9 --weight_decay $WD \
